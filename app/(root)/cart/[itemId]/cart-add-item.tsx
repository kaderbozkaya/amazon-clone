...
export default function CartAddItem({itemId}:{itemId:string}) {
    ...
    }=useCartStore() //sepetteki ürünler(items) ve ürünlerin fiyatı çekilir

    const item=items.find((x)=>x.clientId===itemId)
    if(!item) return notFound() //item dizisinde cliendId si itemId ye eşit olan ürünü arar. eğer ürün bulunmazsa kullanıcı 404 sayfasına yönlendirilir
            ...
            <Card className="w-full rounded-none">
                <CardContent className="flex h-full items-center justify-center gap-3 py-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm">
                        <span className="font-bold">Color:</span>{item.color ?? '-'} {/* tanımlı değilse - ile gösterilir */}
                    </p>
                    <p className="text-sm">
